...
- [ ] **T001** Pure helper `shouldScrollOnMetrics({prevInset, newInset, isScrollMode})` — rising-edge
      over a threshold && not scroll mode (place near the terminal logic or a small util).
- [ ] **T002** `didChangeMetrics`: read bottom view inset, track `_keyboardWasVisible`, and on the
      hidden→visible edge schedule a settled one-shot scroll — *before* the `isAutoResize` early-return.
- [ ] **T003** On the keyboard-show edge, call `_scrollToCaret()` (fallback
      `_ansiTextViewKey.currentState?.scrollToBottom()`); skip when `_terminalMode == TerminalMode.scroll`.
- [ ] **T004** [TDD] Tests: `shouldScrollOnMetrics` (show→true, no-change→false, hide→false,
...
- [ ] **T005** Gate: `flutter analyze --no-fatal-infos` exit 0; `flutter test` ≥ current + new.
