...
## Recent Changes
- 028-translate-docs: Translated the maintained project docs to English — `docs/tmux-mobile-design-v2.md` (the v2 design doc; TypeScript preserved, Japanese prose/comments translated), `docs/ui-guidelines.md`, `docs/coding-conventions.md`, `docs/working_pane.md`, and the user-facing `ios/Runner/Info.plist` permission strings (Face ID / photo / camera). 0 Japanese left in those targets; `README.ja.md` (the JA README), the upstream `001–003` spec archives, `.specify` constitution, and test fixtures left as-is. Docs-only; analyze exit 0, suite 448.
- 027-shell-history: The recent-commands picker now reads the **shell's own history** (`~/.bash_history` / `~/.zsh_history`) over SSH as a **middle tier** — Claude history → shell history → app-recorded — so the ⏱ button is useful in plain bash/zsh panes too (was Claude-only). Pure `parseBashHistory`/`parseZshHistory` (newest-first, deduped; zsh extracts `: ts:dur;cmd`); `ShellHistoryReader` picks the shell from the pane's foreground command, bounded `tail`. +6 tests (suite 448).
