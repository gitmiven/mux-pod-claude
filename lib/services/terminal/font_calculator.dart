...
class FontCalculator {
  ...
  /// Cache of character width ratio (font family → ratio)
  static final Map<String, double> _charWidthRatioCache = {};

  /// Default pane width (in characters)
  static const int defaultPaneWidth = 80;

  /// Minimum pane width (in characters) - narrower panes are clamped to this value
  static const int minPaneWidth = 10;

  /// Calculates font size from screen width and pane character width
  ///
  /// [screenWidth] Available screen width in pixels
  /// [paneCharWidth] Pane horizontal width in characters
  /// [fontFamily] Font family
  /// [minFontSize] Minimum font size (lower bound)
  ///
  /// Returns: Record of (fontSize, needsScroll)
  static FontCalculateResult calculate({
    ...
    // Calculation: fontSize = screenWidth / (paneWidth × charWidthRatio)
    ...
    if (calculatedSize >= minFontSize) {
      ...
      // Horizontal scrolling is needed if below minimum font size
  ...
  /// Measures character width ratio for a font family (using cache)
  ///
  /// For monospace fonts, character width = fontSize × charWidthRatio.
  /// Measures at a base font size of 100 and returns the ratio.
  /// For accuracy, measures 10 characters and takes the average.
  static double measureCharWidthRatio(String fontFamily) {
    ...
    // Even for monospace fonts, metrics can differ subtly between digits and letters,
    // so we include typical patterns and take the average
    ...
    // There have been reports of 0.8 character offset to the left (smaller charWidth),
    // so we considered adding a small buffer (0.01%) to avoid the calculated result being too small.
    // However, by including '0' (standard width) in addition to 'M' (wide), we expect natural correction.
    // If that is still insufficient, offset adjustment will be needed, but we first address this by changing the test characters.
  ...
  /// Calculate terminal display width in pixels
  ///
  /// Used for calculating horizontal scroll container size.
  ...
  /// Measure exact character width for a specified font size
  ///
  /// Accounts for non-linear scaling due to hinting and pixel alignment,
  /// measuring at the actual font size that will be used.
  ...
  /// Converts terminal column position (cursorX) to code unit offset
  ///
  /// Wide characters (Japanese, Chinese, Korean, etc.) occupy 2 columns each,
  /// so tmux's cursor_x (column position) differs from character count.
  /// This function calculates the correct code unit offset from a column position.
  ///
  /// Also accounts for emoji variation selectors (VS16, etc.) and performs
  /// conversion at the grapheme cluster level for accuracy.
  ///
  /// Note: Characters outside the BMP (emoji, etc.) use surrogate pairs
  /// (2 code units), so we return code unit count, not rune count.
  ///
  /// [text] Target text
  ...
  static int columnToCharOffset(String text, int columnPosition) {
    ...
    while (i < runes.length) {
      ...
      final rune = runes[i];
      // Look ahead to next code point (for VS16 check)
      ...
      // Characters outside BMP (U+10000 or above) use 2 code units, others use 1
      codeUnitOffset += _runeCodeUnitCount(rune);
      i++;
    }

    // Skip following zero-width characters (VS16, combining marks, etc.)
    // These are drawn together with the preceding character, so TextPosition
    // needs to point after these characters
    while (i < runes.length) {
      final rune = runes[i];
      final nextRune = (i + 1 < runes.length) ? runes[i + 1] : null;
  ...
  /// Get code unit count for a rune
  ///
  /// Characters in the BMP (Basic Multilingual Plane, U+0000–U+FFFF) use 1 code unit,
  /// characters outside the BMP (U+10000 or above, emoji, etc.) use 2 code units (surrogate pair).
  static int _runeCodeUnitCount(int rune) {
    return rune > 0xFFFF ? 2 : 1;
  }

  /// Get terminal display width for a character (with context)
  ///
  /// If the next code point is VS16 (U+FE0F), returns width 2 for emoji style.
  static int getCharDisplayWidthWithContext(int codePoint, int? nextCodePoint) {
    // Zero-width characters (combining marks, variation selectors, etc.)
    if (_isZeroWidthChar(codePoint)) {
      return 0;
    }

    // If next is VS16 (emoji style), many characters become width 2
    if (nextCodePoint == 0xFE0F) {
      // Characters already width 2 remain unchanged
  ...
  static bool _isZeroWidthChar(int codePoint) {
    ...
    // Skin tone modifiers for Regional Indicator
    if (codePoint >= 0x1F3FB && codePoint <= 0x1F3FF) return true;

    return false;
  }

  /// Determine if a character can display as emoji with VS16
  static bool _canBeEmoji(int codePoint) {
    ...
    // For digit keycap use
    if (codePoint >= 0x0023 && codePoint <= 0x0039) return true; // # 0-9
    // Other text/emoji dual-use symbols
  ...
  /// Get terminal display width for a character (0, 1, or 2)
  ///
  /// Based on Unicode East Asian Width property:
  /// wide characters return 2, normal characters return 1, combining characters etc. return 0.
  static int getCharDisplayWidth(int codePoint) {
    ...
    // Wide character detection (East Asian Width: F, W, A partially)
    ...
    // Everything else is normal width
    return 1;
  }

  /// Calculate maximum number of columns that fit in screen width at specified font size
  ...
  /// Calculate maximum number of rows that fit in screen height at specified font size
  ...
  /// Calculate terminal display width (in columns) for text
  ///
  /// Performs accurate width calculation accounting for variation selectors (VS16, etc.).
