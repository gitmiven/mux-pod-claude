...
class SshKeyService {
  ...
  Future<SshKeyPair> parseFromPem(
    ...
    // Get public key blob (dartssh2's encode returns complete SSH public key blob)
    final publicKeyBlob = keyPair.toPublicKey().encode();
    // Calculate fingerprint directly from blob (do not re-wrap)
    ...
    if (type == 'ssh-ed25519') {
      ...
      // RSA bit length inferred from public key
    ...
    return SshKeyPair(
      type: keyType,
      privateKeyBytes: Uint8List(0), // private key bytes not needed during parsing
      publicKeyBytes: publicKeyBlob,
      fingerprint: fingerprint,
  ...
  /// Check if key is encrypted with passphrase
  ...
  Uint8List _buildPublicKeyBlob(String keyType, Uint8List publicKeyBytes) {
    if (keyType == 'ssh-ed25519') {
      // For Ed25519, public key is 32 bytes
  ...
  Uint8List _rsaPrivateKeyToBytes(pc.RSAPrivateKey privateKey) {
    // For simplification, return byte representation of modulus
    return _bigIntToBytes(privateKey.modulus!);
  }

  String _buildEd25519Pem(
      Uint8List privateKey, Uint8List publicKey, String comment) {
    // Build OpenSSH format Ed25519 private key PEM
    // For simplification, return in format readable by dartssh2
    final buffer = BytesBuilder();
  ...
  Uint8List _encodeAsn1Integer(BigInt value) {
    ...
    var bytes = _bigIntToBytes(value);
    // If high bit is set, prepend 0x00 for sign bit
    if (bytes.isNotEmpty && (bytes[0] & 0x80) != 0) {
      bytes = Uint8List.fromList([0, ...bytes]);
    }
    // For zero, use single byte 0
    if (bytes.isEmpty) {
