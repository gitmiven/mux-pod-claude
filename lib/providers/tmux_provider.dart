...
/// Notifier for managing tmux sessions
class TmuxNotifier extends Notifier<TmuxState> {
  ...
  void setActiveSession(String sessionName) {
    // Auto-select the first active window and pane in the session
  ...
  void setActiveWindow(int windowIndex) {
    // Auto-select the first active pane in the window
  ...
  void updateCursorPosition(String paneId, int x, int y) {
    // Skip if no changes to avoid deep copy
  ...
  /// Set active session/window/pane in bulk
  void setActive({
    String? sessionName,
    int? windowIndex,
    int? paneIndex,
  ...
  /// Get current polling target tmux target string
