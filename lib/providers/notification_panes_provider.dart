...
/// State of the notification panes list
...
/// Notifier that manages the notification panes list
class AlertPanesNotifier extends Notifier<AlertPanesState> {
  ...
  /// Clear the window flag on tmux side (select the target window with select-window and then return to the previous window)
  Future<void> clearWindowFlag(AlertPane alert) async {
    ...
    try {
      ...
      // Select the target window to clear the flag and return to the original window
